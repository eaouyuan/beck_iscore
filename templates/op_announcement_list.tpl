...
<{if $all}>
    ...
    <{foreach from=$all key=i item=its}>
        <tr> 
            ...
            <td style="line-height: 1.4em;" class="text-left clickable-row" data-href="<{$xoops_url}>/modules/beck_iscore/index.php?op=announcement_show&sn=<{$its.sn}>">
                <{if $its.top=='1'}><span class="badge" style="background-color:blue;font-weight:normal;color:white;text-shadow:none;">置頂</span><{/if}>
                <{$its.title}>
            </td>
            <td class="text-center clickable-row text-nowrap" data-href="<{$xoops_url}>/modules/beck_iscore/index.php?op=announcement_show&sn=<{$its.sn}>"><{$its.start_date}></td>

<form name="mentor_tutor_list" id="mentor_tutor_list" action="tchstu_mag.php" method="get">
    <h2 class="mb-3">導師輔導紀錄</h2>
    <div class="col">
...
<{if $all}>
    ...
    <{foreach from=$all key=i item=its}>
        <tr> 
            ...
            <th class="text-center"><{$its.dep_name}></th>
            <th class="text-center"><{$its.tutor_name}></th>
            <th class="text-center"><{$its.stu_name}></th>
            <th class="text-center"><{$its.count}></th>
            <th class="text-center">
                <a href="<{$xoops_url}>/modules/beck_iscore/tchstu_mag.php?op=mentor_tutor_show&year=<{$its.year}>&term=<{$its.term}>&stu_sn=<{$its.student_sn}>&tea_uid=<{$its.tea_uid}>" class="btn btn-primary btn-sm mr-2">觀看</a>
            </th>
        </tr>
    <{/foreach}>
        </tbody>
...
<script type="text/javascript">
    $(document).ready(function($){
        $('#cos_year').change(function(e){
            document.forms["mentor_tutor_list"].submit();
        });
